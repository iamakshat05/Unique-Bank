...
const CreditCard = () => {
  ...
  const handleFetchData = async () => {
    setIsLoading(true);
    try {
      const response = await getCreditCards(config);
...
const filterData = () => {
    ...
    const filteredData = data.filter((row) => {
      const { cardHolderName, cardNumber } = row;
      // const query = searchQuery.toLowerCase();
      // console.log(occupied,"occupied")
  
      return (
        (cardNumber && cardNumber.toString().toLowerCase().includes(searchQuery)) ||
        (cardHolderName && cardHolderName.toString().toLowerCase().includes(searchQuery))
...
const handleDeleteDialogeBox=()=>{
  ...
  const handleApplyCreditCard =(e)=>{
    if(e.data){
      ...
      handleFetchData()
      toast.success("credit card Applied ")

      return
    }else{

      toast.error("Credit Card not Applied!")
    }
  }
  const handleSetLimit =(e)=>{
    if(e.data){
      ...
      setSelectedRow(null)
      handleFetchData()
      toast.success("Set Limit Success ")

      return
    }else{

      toast.error("Limit Not Set !")
    }
  }
...
<div >
      ...
      <input
        type="text"
        placeholder="Search by Card Holder Name, Card Number"
        value={searchQuery}
  ...
  <Table>
    <thead style={styles.tableHeading}>
      <tr>
        ...
        <TableCellStyled onClick={() => handleSort("balance")}>
          Balance
        ...
        <TableCellStyled>Card Holder Name</TableCellStyled>
        <TableCellStyled>Card Number</TableCellStyled>
        <TableCellStyled>Card Type</TableCellStyled>
        <TableCellStyled>Credit Limit</TableCellStyled>
        <TableCellStyled>CVV</TableCellStyled>
        <TableCellStyled>Expiry Date</TableCellStyled>
        <TableCellStyled>Interest Rate</TableCellStyled>
        <TableCellStyled>Age</TableCellStyled>
        <TableCellStyled>Income</TableCellStyled>
        <TableCellStyled>Due Amount</TableCellStyled>
        <TableCellStyled>Last Payment Date</TableCellStyled>
...
onMouseLeave={() => setHoveredRowId(null)}
        ...
        >
          ...
          <TableCellStyled>{row.cardHolderName}</TableCellStyled>
          <TableCellStyled>{row.cardNumber}</TableCellStyled>
          <TableCellStyled>{row.cardType}</TableCellStyled>
          <TableCellStyled>{row.creditLimit}</TableCellStyled>
          <TableCellStyled>{row.cvv}</TableCellStyled>
          <TableCellStyled>{row.expiryDate}</TableCellStyled>
          <TableCellStyled>{row.interestRate}</TableCellStyled>
          <TableCellStyled>{row.age}</TableCellStyled>
          <TableCellStyled>{row.income}</TableCellStyled>
          <TableCellStyled>{row.dueAmount}</TableCellStyled>
          <TableCellStyled>{row.lastPaymentDate}</TableCellStyled>
          {roles.includes("ROLE_ADMIN") || roles.includes("ROLE_MANAGER") ? (
  <TableCellStyled>
    <IconButton>
      <TuneIcon onClick={() => handleEditClick(row)} />
    </IconButton>
  </TableCellStyled>
) : null}
      ...
      <Dialog open={Boolean(selectedRow)} onClose={() => setSelectedRow(null)}>
        ...
        <p>Card Number: {selectedRow.cardNumber}</p>
        <p>Card Holder: {selectedRow.cardHolder}</p>
        <p>Expiry Date: {selectedRow.expiryDate}</p>
        <p>CVV: {selectedRow.cvv}</p>
            </div>
          )}
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          {openEditDialog && <SetCreditLimit editedUser={editedUser} handleSetLimit ={(e)=>handleSetLimit(e)} />}
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={openAddDialog} onClose={handleAddDialogClose}>
        ...
        <DialogContent>{openAddDialog && <ApplyCreditCard handleApplyCreditCard ={(e)=>handleApplyCreditCard(e)} />}</DialogContent>
        <DialogActions>
